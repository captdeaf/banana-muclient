...
#define readb(b) if (recv(w->fd, &b, 1, 0) < 1) return -1;
int
iac_do(World *w, int b) {
  switch (b) {
  case _LINEMODE:
    write(w->fd, IAC WONT LINEMODE, 3);
    break;
  case _TTYPE:
    write(w->fd, IAC WILL TTYPE, 3);
    break;
  case _NAWS:
...
#define readb(b) if ((ret = recv(w->fd, &b, 1, 0)) < 1) goto exit_sequence
  while (1) {
    readb(b);
    switch (b) {
    case '\r':
